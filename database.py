...
class Database:
    ...
    def is_admin(self, society_id, chat_id, admin_chat_id_env=""):
        # Pehle environment variable check karo
        if admin_chat_id_env and str(chat_id) == admin_chat_id_env:
            return True
    ...
    def add_resident(self, flat_id, name, mobile=None, telegram_chat_id=None, added_by="admin"):
        with self.get_conn() as conn:
            ...
            if not existing:
                conn.execute(
                    "INSERT INTO residents (flat_id, name, mobile, telegram_chat_id, added_by, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                    (flat_id, name, mobile, telegram_chat_id, added_by, datetime.now().strftime("%Y-%m-%d"))
                )
                return True
        return False
    ...
    def update_resident_telegram(self, telegram_chat_id, flat_number, society_id):
        ...
        with self.get_conn() as conn:
            conn.execute(
                "UPDATE residents SET telegram_chat_id=?, is_verified=1 WHERE flat_id=? AND is_verified=0",
    ...
    def get_resident_by_telegram(self, telegram_chat_id):
        with self.get_conn() as conn:
            row = conn.execute(
                "SELECT r.*, f.flat_number, f.society_id FROM residents r JOIN flats f ON r.flat_id=f.id WHERE r.telegram_chat_id=?",
    ...
    def get_all_residents(self, society_id):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT r.id, r.name, r.mobile, r.telegram_chat_id, r.is_verified, f.flat_number FROM residents r JOIN flats f ON r.flat_id=f.id WHERE f.society_id=? ORDER BY f.flat_number",
                (society_id,)
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_residents_with_telegram(self, society_id):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT r.*, f.flat_number FROM residents r JOIN flats f ON r.flat_id=f.id WHERE f.society_id=? AND r.telegram_chat_id IS NOT NULL",
                (society_id,)
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def mark_paid(self, flat_id, month, payment_mode=None):
        with self.get_conn() as conn:
            conn.execute(
                "UPDATE maintenance SET status='paid', paid_date=?, payment_mode=? WHERE flat_id=? AND month=?",
    ...
    def get_maintenance_status(self, society_id, month):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT f.flat_number, r.name, r.mobile, r.telegram_chat_id, m.amount, m.status, m.paid_date FROM maintenance m JOIN flats f ON m.flat_id=f.id LEFT JOIN residents r ON r.flat_id=f.id WHERE f.society_id=? AND m.month=? ORDER BY f.flat_number",
                (society_id, month)
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_pending_list(self, society_id, month):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT f.flat_number, r.name, r.mobile, r.telegram_chat_id, m.amount FROM maintenance m JOIN flats f ON m.flat_id=f.id LEFT JOIN residents r ON r.flat_id=f.id WHERE f.society_id=? AND m.month=? AND m.status='pending' ORDER BY f.flat_number",
                (society_id, month)
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_summary(self, society_id, month):
        with self.get_conn() as conn:
            total = conn.execute(
                "SELECT COUNT(*) as total, SUM(amount) as total_amount FROM maintenance m JOIN flats f ON m.flat_id=f.id WHERE f.society_id=? AND m.month=?",
                (society_id, month)
            ).fetchone()
            paid = conn.execute(
                "SELECT COUNT(*) as paid_count, SUM(amount) as paid_amount FROM maintenance m JOIN flats f ON m.flat_id=f.id WHERE f.society_id=? AND m.month=? AND m.status='paid'",
                (society_id, month)
            ).fetchone()
    ...
    def add_alert(self, society_id, alert_type, title, message, created_by=None):
        with self.get_conn() as conn:
            conn.execute(
                "INSERT INTO alerts (society_id, alert_type, title, message, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (society_id, alert_type, title, message, created_by, datetime.now().strftime("%Y-%m-%d %H:%M"))
            )

    def get_recent_alerts(self, society_id, limit=5):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT * FROM alerts WHERE society_id=? ORDER BY created_at DESC LIMIT ?",
                (society_id, limit)
            ).fetchall()
        return [dict(r) for r in rows]
    ...
    def get_pending_complaints(self, society_id):
        with self.get_conn() as conn:
            rows = conn.execute(
                "SELECT c.*, f.flat_number FROM complaints c JOIN flats f ON c.flat_id=f.id WHERE f.society_id=? AND c.status='pending' ORDER BY c.created_at DESC",
                (society_id,)
            ).fetchall()
        return [dict(r) for r in rows]
